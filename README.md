...
(2026-08-27 and 08-28 have two snapshots each), which is the spread a
hosted scheduler produces under load. The history accumulates from the
day the repo went live, so every later study has an archive it can
replay.
...
rather than taken on trust. It moves as the venue's catalog moves, which
is why it is quoted against a named snapshot instead of as a standing
fact.

Recorder v1, which wrote the snapshots held in `data/` on `main`,
recorded the first 2,000 Kalshi events (about 14,000 markets, dominated
by the two midterm ladder families), Polymarket as served, and Manifold's
1,000 newest markets. Those blobs are kept exactly as recorded and load
through the same loader.

## Storage

Snapshots live in two places, and both are append-only:
...
3. **Ladder monotonicity.** Threshold markets on one variable must
   ...
   strike's ask is incoherence beyond the spread. What counts as a
   ladder is the parser in `pmlab/ladders.py`, which covers the families
   that actually appear in the archive (`Above NK`, `Above N%`,
   ...
   captured them. Coverage decides the count, which is why every ladder
   number below is regenerated by CI and carries the date it was
   measured on.
...
disagrees with `results/summary.json`. Every figure below is regenerated,
not typed.
...
`tests/test_incremental.py` pins it in both directions. The cache key
carries a hash of every module in `pmlab/`, so a change to the code
starts a fresh cache rather than serving an answer the code would no
longer give.
...
from 2026-08-23 to 2026-09-18, spanning
26.7 <!-- results:archive.days_spanned --> days at a realised cadence
median of 3.5 <!-- results:archive.median_gap_hours --> h.
125 <!-- results:archive.schema1_snapshots --> of them come from
recorder v1 and 38 <!-- results:archive.schema2_snapshots --> from v2.

### Ladders

The two recorders see different universes: v1 covers the first 2,000
events (about 14,000 markets, dominated by the two midterm ladder
families), v2 sweeps the whole open catalog (about 56,000). Pooling them
counts two different experiments as one, so the headline is split.
...
cents behind. Charging the unrounded `0.07 p (1-p)` instead understates
the fee on a one-cent contract by a factor of fourteen, which is the
whole distance between "a one-cent inversion survives" and "it does not".
...
counts, gross and net. The rates come from that feed rather than from the
fee-schedule PDF, which answers HTTP 429 from here.
...
weeks of v2 recording before it is a result. All six pairs are in the
catalog at all only because v2 sweeps past the 2,000-event cap. Kalshi's
own fee feed prices none of these three series below the general rate, so
these net edges are exactly what they say.

Two families of false positive are excluded by construction, both worth
naming because the venue's structured fields invite them: `KXNFLSPREAD`
...
`(event_ticker, strike_type)` alone reads a 35c and a 46c "arbitrage" out
of markets that can both settle YES. Rungs therefore also have to agree
on the shape of their sub-title, and a rung whose sub-title is a bare
number is refused.
...
95 <!-- results:polymarket_complement.snapshots_numeric_era --> snapshots
of the `liquidityNum` era. Those quotes sit in the thin, often
already-expired rows that a text sort of `liquidity` puts at the top of
the page, so they describe that slice of the archive rather than
Polymarket's book.
...
163 <!-- results:archive.snapshots --> snapshots, covering
...
Polymarket pairs and 63,066 <!-- results:buckets.screened_total --> screened
...
string-sorted era, the slice a text sort of `liquidity` selected, rather
than in the venue's liquid book. The bucket-sum candidates are not
arbitrage at any fee: the open-universe trap named above disqualifies them
before a fee model is reached.

The surviving ladder pairs live in the part of the catalog only the
untruncated sweep reaches: the recorder-v1 slice,
...
Two rules keep the file's own contents checkable, since an unverified
pair is never priced and so nothing downstream would catch a bad key.
Every Kalshi ticker in it is read out of a recorded snapshot rather than
constructed: the Nobel leg is `KXNOBELPEACE-26-ELO`, taken from the
2026-09-11T20:03Z catalog, where twenty-one `KXNOBELPEACE` markets are
listed. And `pmlab/basis.py` refuses any leg key containing
`TODO`/`TBD`/`FIXME`, so a placeholder cannot sit inside the pair count.
No NBER recession pair is listed, because no recorded Polymarket or
PredictIt row settles on the NBER dating committee's announcement, and
the file says so where the pair would go.
...
are Polymarket rows from the string-sorted era, the thin and often
already-expired markets a text sort of `liquidity` puts at the top, against
769 <!-- results:calibration.composition.by_era.numeric --> from the
`liquidityNum` era. Every incoherent Polymarket quote in this archive is
in that same era, so about a fifth of the headline sample comes from the
slice of the archive this repo trusts least. The scores themselves live in
...
media calls are outside recorder v1's 2,000-event cap and enter the
archive from recorder v2 onward. Manifold resolves continuously but is
play money. So the table is rebuilt on every run and gets its first
serious Kalshi block over the winter.
...
gets reported in-sample. It carries no numbers because it has not run.
Whether it runs properly or not at all is a decision for after the
winter calibration tables.

## Phase 4 (planned): model vs market on financial events

Kalshi lists range markets on equity indexes (KXINXY, KXINXDIRY and the
daily KXNASDAQ100U family are in the archive). The BTC range series enter
the archive with recorder v2: across all
125 <!-- results:archive.schema1_snapshots --> v1 snapshots the only BTC ticker is
`KXTREASBUYBTC`, a custom-strike market, because the range series sit
beyond the v1 2,000-event cap. The newest v2
...
- Polymarket rows from 2026-08-23 through the 2026-09-01T16:44Z snapshot
  are ordered by `liquidity` as a string, because the venue served the
  field as text: median liquidity about $99, 12% of rows already past
  their end date. The loader tags those rows `era="string_sorted"`; the
  blobs are kept as recorded.
- Recorder v1 reads 10 pages of 200 Kalshi events, so every v1 snapshot
  holds exactly 2,000 events and the per-race 2026 winner series (for
  example KXHOUSE*) are not in them. Recorder v2 sweeps the full open
  catalog.
- Manifold under recorder v1 is the 1,000 most recently created markets;
  under v2 it is the 1,000 most liquid.
- Recorder v1 keeps only Kalshi markets with a bid or an ask, so an event
...
python -m pmlab.replay                # every screen over every snapshot, into results/
